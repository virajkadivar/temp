(function($) { 
	...
	/**
Core script to handle the entire template and core functions
**/
var Pexp = function(){
	/* Search Bar ============ */
	var siteUrl = '';
		...
		jQuery(window).on('scroll', function () {
			if(jQuery('.sticky-header').length > 0){
				...
				if ($(window).scrollTop() > menu.offset().top) {
					...
					$('.header-Pexp .container > .logo-header .logo').attr('src','images/logo.png');
				} else {
					menu.removeClass('is-fixed');
					$('.header-Pexp .container > .logo-header .logo').attr('src','images/logo-white.png')
				}
			}
		});
